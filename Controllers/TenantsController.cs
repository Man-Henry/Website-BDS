...
namespace Website_QLPT.Controllers
{
    ...
    public class TenantsController : Controller
    {
        ...
        public async Task<IActionResult> Index(string? search, int? page)
        {
            ...
            var list = await orderedQuery.Skip((pageNumber - 1) * pageSize).Take(pageSize).ToListAsync();
            return View(new StaticPagedList<Tenant>(list, pageNumber, pageSize, totalCount));
        }
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            if (tenant != null)
            {
                var hasContracts = await _context.Contracts.AnyAsync(c => c.TenantId == id);
                if (hasContracts)
                {
                    TempData["Error"] = "Không thể xóa người thuê này vì đang có hợp đồng liên kết.";
                    return RedirectToAction(nameof(Index));
                }
