...
namespace Website_QLPT.Controllers
{
    ...
    public class MaintenanceTicketsController : Controller
    {
        ...
        private readonly Microsoft.AspNetCore.SignalR.IHubContext<Website_QLPT.Hubs.NotificationHub> _hubContext;

        public MaintenanceTicketsController(
            ApplicationDbContext context,
            IWebHostEnvironment env,
            ICurrentTenantService currentTenantService,
            Microsoft.AspNetCore.SignalR.IHubContext<Website_QLPT.Hubs.NotificationHub> hubContext)
        {
            _context = context;
            _env = env;
            ...
            [FromForm(Name = "image")] IFormFile? legacyImageFile)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (uploadedImage != null && uploadedImage.Length > 0)
                {
                    ...
                    if (!allowedExt.Contains(ext) || !uploadedImage.ContentType.StartsWith("image/"))
                    {
                        ModelState.AddModelError("", "File ảnh không hợp lệ. Vui lòng tải lên file định dạng hình ảnh hợp lệ (jpg, png, webp, gif).");
                        var activeContractsErr = await _context.Contracts.Include(c => c.Room)
                            .Where(c => c.TenantId == tenant.Id && c.Status == ContractStatus.Active)
                            .Select(c => new { c.Id, RoomName = c.Room!.Name }).ToListAsync();
                        ViewBag.ContractId = new SelectList(activeContractsErr, "Id", "RoomName", ticket.ContractId);
                        ViewData["Layout"] = "_TenantLayout";
                        return View(ticket);
                    }
                ...
                // Gửi Notification tới Chủ trọ
                var contractInfo = await _context.Contracts
                    .Include(c => c.Room).ThenInclude(r => r!.Property)
                    .FirstOrDefaultAsync(c => c.Id == ticket.ContractId);
                var ownerId = contractInfo?.Room?.Property?.OwnerId;
                if (!string.IsNullOrEmpty(ownerId))
                {
                    var notif = new AppNotification
                    {
                        ...
                        Message = $"Sự cố '{ticket.Title}' vừa được báo cáo từ phòng {contractInfo?.Room?.Name}.",
                        Url = "/MaintenanceTickets",
                    ...
                    await _hubContext.Clients.Group(ownerId).SendAsync("ReceiveNotification", new
                    {
                        id = notif.Id,
                        title = notif.Title,
