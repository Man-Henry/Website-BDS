...
namespace Website_QLPT.Controllers
{
    ...
    public class ReportsController : Controller
    {
        ...
        public async Task<IActionResult> Index(int? year)
        {
            ...
            // Get all invoices for the owner
            ...
            var availableYears = await invoicesQuery
                .Select(i => i.Year)
                .Distinct()
            ...
            var yearInvoices = await invoicesQuery
                .Where(i => i.Year == selectedYear)
                .ToListAsync();

            var monthlyPaidRevenue = new List<decimal>(new decimal[12]);
            var monthlyUnpaidRevenue = new List<decimal>(new decimal[12]);

            foreach (var inv in yearInvoices)
            {
                // Note: Month in Invoice model is 1-12. Array index is 0-11.
                var index = inv.Month - 1;
                if (index >= 0 && index < 12)
                {
                    if (inv.Status == InvoiceStatus.Paid)
                    {
                        monthlyPaidRevenue[index] += inv.TotalAmount;
                    }
                    else
                    {
                        monthlyUnpaidRevenue[index] += inv.TotalAmount;
                    }
                }
            }
            ...
            return View(new ReportsIndexViewModel
            {
                ...
                MonthlyUnpaidRevenue = monthlyUnpaidRevenue
            });
        }
